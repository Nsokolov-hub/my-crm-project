...
def login(
    ...
    if valid and user and user.mfa_enabled:
        ...
        valid = bool(
            totp and body.otp and totp.verify(body.otp, valid_window=0) and step > user.mfa_last_step
        )
        if valid:
    ...
    return {
        ...
        "csrf_token": csrf,
        "permissions": user_view(db, user)["permissions"],
    }
...
def me(request: Request, user: User = Depends(current_user), db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    return {
        "user": data,
        "csrf_token": request.state.auth_session.csrf_token,
...
def mfa_setup(user: User = Depends(current_user), db: Session = Depends(get_db)) -> dict[str, str]:
    ...
    return {
        "secret": secret,
        "uri": pyotp.TOTP(secret).provisioning_uri(user.email, issuer_name="Реактив CRM"),
    }
...
def mfa_enable(
    ...
    if not user.mfa_secret or not pyotp.TOTP(cipher().decrypt(user.mfa_secret.encode()).decode()).verify(
        ...
        raise DomainError(
            "OTP_INVALID", "Неверный код. Проверьте время в приложении аутентификации.", 422, "otp"
        )
...
def change_password(
    ...
    db.execute(
        update(AuthSession)
        .where(AuthSession.user_id == user.id, AuthSession.id != request.state.auth_session.id)
        .values(revoked=True)
    )
...
def assign_grants(
    db: Session, grants: list[GrantInput], *, role_id: str | None = None, user_id: str | None = None
) -> None:
    ...
    db.execute(
        delete(PermissionGrant).where(
            PermissionGrant.role_id == role_id if role_id else PermissionGrant.user_id == user_id
        )
    )
    for g in grants:
    ...
    if body.active is False:
        ...
        pending = (
            db.scalar(
                select(func.count())
                .select_from(Task)
                .where(Task.assignee_id == row.id, Task.status.in_(["assigned", "in_progress"]))
            )
            or 0
        )
        if pending and (not target or not target.active or target.id == row.id):
            raise DomainError(
                "REASSIGN_REQUIRED", "Выберите сотрудника для незавершённых задач", 422, "reassign_to"
            )
        if target:
            for model, col in [
                (Task, Task.assignee_id),
                (CRMRequest, CRMRequest.owner_id),
                (Counterparty, Counterparty.owner_id),
            ]:
        ...
        "items": [
            {
                **serialize(r),
                "grants": [
                    {"code": g.code, "scope": g.scope, "allow": g.allow}
                    for g in db.scalars(select(PermissionGrant).where(PermissionGrant.role_id == r.id))
                ],
            }
            for r in rows
        ],
...
def set_setting(
    ...
    if body.key in SETTING_META:
        meta = SETTING_META[body.key]
        schema, req_perm = meta.schema, meta.permission
        if not meta.is_supported:
            raise DomainError("UNSUPPORTED_SETTING", "Данная настройка пока не поддерживается сервером и не будет сохранена.", 400)
    ...
    if any(x in body.key.lower() for x in ("secret", "password", "token", "api_key")):
        raise DomainError(
            "SECRET_SETTING_FORBIDDEN", "Секреты задаются через защищённую конфигурацию сервера", 422, "key"
        )

    advisory(db, f"setting:{body.key}")
    row = db.scalar(
        select(AppSetting)
        .where(AppSetting.key == body.key, AppSetting.status == "published")
        .with_for_update()
    )
...
def setup_wizard(
    ...
    if db.scalar(
        select(AppSetting).where(AppSetting.key == "company_name", AppSetting.status == "published")
    ):
    ...
    _add_setting(
        "loss_reasons", {"reasons": ["price_too_high", "went_to_competitor", "no_budget", "timing", "other"]}
    )
