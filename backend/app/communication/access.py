...
FILE_PERMISSIONS = {
    "purchase": "finance.purchase.read",
    "calculation": "finance.calculations.read",
    "reward": "finance.reward.read",
    "profit": "finance.profit.read",
}
ENTITY_COLUMNS = {
    "request": "request_id",
    "client": "client_id",
...
def check_wave(db: Session, user: User, entity_id: str):
    ...
    if row is None or not (
        ...
        or scope_for(db, user, "waves.write") == "all"
...
def check_chat(
    db: Session, user: User, entity_id: str, *, for_update: bool = False
    ...
    member = db.scalar(
        select(ChatMember).where(
            ChatMember.chat_id == entity_id, ChatMember.user_id == user.id, ChatMember.active.is_(True)
        )
    )
...
def file_view(row: FileRecord) -> dict:
    ...
    # Expose scan status for UI display (quarantined/clean/infected/scan_failed)
    # but keep raw scan_result detail hidden
